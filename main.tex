...
To illustrate, with $\gamma=2$ and $\alpha=0.25$, an easy negative ($\hat{y}=0.05$) has its loss contribution reduced by ${\sim}500\times$ compared to BCE, while a hard positive ($\hat{y}=0.3$) is reduced by only ${\sim}8\times$. This differential down-weighting shifts gradient mass toward informative examples.
...
\label{sec:baselines}
We compare non-personalized popularity ranking (PopRank) as a lower bound, traditional methods including item-based and user-based KNN with cosine similarity and SVD with MSE loss, and neural methods including MF and NeuMF trained with BCE, BPR, and Focal Loss. An NeuMF-$\alpha$BCE control variant isolates the focusing mechanism's contribution.
...
For traditional methods, KNN uses neighborhood sizes $K \in \{10, 20, 40, 80\}$ with cosine and Pearson similarity metrics, while SVD tests latent dimensions $\{32, 64, 128\}$ and regularization strengths $\{0.001, 0.01, 0.1\}$. Neural methods search over embedding sizes $\{32, 64, 128\}$, MLP architectures $\{[64,32], [128,64,32]\}$, learning rates $\{0.0001, 0.001, 0.01\}$, and batch sizes $\{256, 512, 1024\}$. Focal Loss parameters span $\gamma \in \{0, 0.5, 1.0, 2.0, 3.0\}$ and $\alpha \in \{0.25, 0.5, 0.75\}$, where $\gamma = 0$ reduces to $\alpha$-balanced BCE, enabling direct comparison for H3 testing. Based on the alpha-sampling interaction analysis, we anticipate optimal $\alpha$ values will exceed 0.25 for high sampling ratios.
...
The effective class weight ratio $R_{\text{eff}}$ varies dramatically with these parameters: at 1:4 sampling, $\alpha = 0.25$ yields $R_{\text{eff}} = 12:1$, while at 1:50 sampling, the same $\alpha$ produces $R_{\text{eff}} = 150:1$, potentially overwhelming the positive signal entirely. Balanced weighting ($\alpha = 0.5$) at 1:50 sampling still results in $R_{\text{eff}} = 50:1$. To achieve $R_{\text{eff}} = 1$ at 1:50 sampling would require $\alpha \approx 0.98$. This analysis motivates our grid search over $\alpha$ values and explains why the default $\alpha = 0.25$ from computer vision applications may require adjustment for recommendation tasks with high negative sampling ratios.
...
We conduct ablations along five dimensions: (1)~loss function (BCE, BPR, Focal Loss) on fixed NeuMF; (2)~Focal Loss components, varying $\gamma \in \{0, 0.5, 1.0, 2.0, 3.0\}$ and $\alpha \in \{0.25, 0.5, 0.75\}$; (3)~mechanism isolation comparing full Focal Loss ($\gamma > 0$) against $\alpha$-BCE ($\gamma = 0$) to test H3; (4)~architecture (GMF, MLP, NeuMF); and (5)~negative sampling ratio (1:4, 1:10, 1:50) to test H2.
...
To test H3 (Mechanism), we compared full Focal Loss ($\gamma > 0$) against $\alpha$-balanced BCE ($\gamma = 0$) with matched $\alpha$ values at 1:10 sampling. The BCE baseline achieved NDCG@10 of 0.0575 and HR@10 of 0.1157. With class reweighting alone, $\alpha$-BCE ($\gamma = 0$, $\alpha = 0.25$) achieved NDCG@10 of 0.0600 (+4.3\%) and HR@10 of 0.1231. However, full Focal Loss ($\gamma = 2$, $\alpha = 0.25$) achieved only NDCG@10 of 0.0556 ($-3.3\%$) and HR@10 of 0.1062.
...
Based on experimental evidence, H1 (Efficacy) was \textbf{supported}: Focal Loss achieved +10.0\% NDCG@10 at 1:50 sampling with $p = 0.0625$ and Cohen's $d = 2.632$. H2 (Robustness) was \textbf{partially supported}: Focal Loss outperformed BCE on 2 of 3 sampling ratios but failed at 1:10. H3 (Mechanism) was \textbf{not supported} at moderate imbalance, as $\alpha$-BCE outperformed full Focal Loss at 1:10.
