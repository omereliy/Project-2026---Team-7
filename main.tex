...
To illustrate how Focal Loss rebalances learning toward informative examples, consider a toy recommendation scenario with 5 users and 10 items containing 10 positive interactions and 40 negative entries, yielding an imbalance ratio $\rho = 4:1$. Consider two training examples after several epochs of training:
...
The ablation studies compare BCE ($\gamma=0$, $\alpha=0.5$) as baseline, $\alpha$-BCE ($\gamma=0$, varying $\alpha \in \{0.25, 0.5, 0.75\}$) for class weighting effects, and full Focal Loss ($\gamma \in \{0.5, 1, 2, 3\}$) against the $\gamma=0$ control to isolate the focusing mechanism.
