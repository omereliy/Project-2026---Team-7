...
Recommender systems have become the cornerstone of modern digital platforms, guiding users through vast catalogs of products, content, and services. Unlike explicit feedback systems where users directly express preferences through ratings, contemporary recommender systems increasingly rely on implicit feedback---behavioral signals such as clicks, purchases, and viewing history---which are abundant but inherently noisy. A user's failure to interact with an item does not necessarily indicate disinterest; it may simply reflect unawareness of the item's existence. This fundamental ambiguity, combined with the observation that users typically interact with only a minuscule fraction of available items, creates a severe class imbalance problem: the ratio of unobserved to observed interactions routinely exceeds 20:1 and can surpass 100:1 in sparse datasets.

Standard training objectives for neural recommendation models handle this imbalance through heuristic negative sampling, randomly selecting a small subset of unobserved items to serve as negative examples. While computationally efficient, this approach is statistically flawed. Binary Cross-Entropy (BCE) and Bayesian Personalized Ranking (BPR) losses treat all samples equally regardless of prediction confidence, causing easy negatives---items clearly irrelevant to a user---to dominate gradient updates. The model expends learning capacity on trivially classified examples while receiving insufficient signal from the challenging boundary cases that truly matter for recommendation quality. Hard negative mining strategies address this partially but introduce significant computational overhead and require careful tuning.
...
\begin{itemize}
    \item A systematic evaluation of Focal Loss for collaborative filtering on standard implicit feedback benchmarks (MovieLens 100K and 1M), demonstrating its effectiveness outside specialized bioinformatics domains.
    \item Comprehensive hyperparameter analysis through ablation studies of the focusing parameter $\gamma$ and class-balancing parameter $\alpha$, providing practical guidance for practitioners.
    \item Architecture-agnostic validation on both Matrix Factorization and NeuMF, testing whether Focal Loss benefits are specific to neural architectures.
\end{itemize}

The remainder of this paper is organized as follows. Section~\ref{sec:background} introduces foundational concepts in collaborative filtering, neural recommendation, and Focal Loss. Section~\ref{sec:related-work} surveys related work and identifies the research gap. Section~\ref{sec:methodology} presents our methodology, including formal problem definition and the application of Focal Loss to recommendation. Section~\ref{sec:experiments} details the experimental design, including datasets, baselines, evaluation metrics, and ablation studies.
...
For typical recommendation datasets, $\rho$ ranges from 20:1 to over 100:1, meaning negative samples dominate the training data by orders of magnitude. This asymmetry is compounded by the \textit{Missing Not At Random} (MNAR) problem~\cite{marlin2009collaborative}: the absence of an interaction does not indicate negative preference but merely that the user has not encountered or considered the item. The recommendation task is to learn a scoring function $\hat{y}_{ui} = f(u, i; \Theta)$ that predicts the likelihood of user $u$ interacting with item $i$, where $\Theta$ represents model parameters. The objective is to rank items such that those with higher predicted scores are more likely to be relevant to the user.
...
We evaluate Focal Loss on two collaborative filtering architectures to assess whether its benefits are architecture-agnostic.
...
\begin{enumerate}
    \item \textbf{Systematic Evaluation of Focal Loss for General-Purpose Recommendation:} Unlike prior work applying Focal Loss to specialized domains~\cite{meng2022weighted}, we evaluate its effectiveness on standard recommendation benchmarks (MovieLens 100K and 1M) with implicit feedback.
    \item \textbf{Comprehensive Hyperparameter Analysis:} We conduct ablation studies of the focusing parameter $\gamma$ and class-balancing parameter $\alpha$, providing practical guidance for practitioners on configuring Focal Loss for recommendation tasks.
    \item \textbf{Architecture-Agnostic Validation:} By testing Focal Loss on both MF and NeuMF, we determine whether performance gains are specific to neural architectures or generalize across model complexity levels.
\end{enumerate}
...
\begin{description}
    \item[H1 (Focal Loss improves NeuMF):] NeuMF trained with Focal Loss achieves statistically significantly higher HR@10 and NDCG@10 compared to NeuMF trained with Binary Cross-Entropy loss.
    \item[H2 (Optimal focusing parameter):] There exists an optimal value of the focusing parameter $\gamma$ in the range $[0.5, 3.0]$ that maximizes recommendation performance.
    \item[H3 (Architecture-agnostic benefit):] Focal Loss improvements generalize to Matrix Factorization, demonstrating the benefit is not architecture-specific.
\end{description}

\subsection{Datasets}
\label{sec:datasets}

We conduct experiments on two benchmark datasets, summarized in Table~\ref{tab:datasets}. MovieLens 100K and 1M~\cite{harper2015movielens} are standard benchmarks directly comparable with prior NCF literature, providing varying scales while maintaining moderate density levels suitable for evaluating loss function effects.
...
We compare the following models, summarized in Table~\ref{tab:models}. Traditional collaborative filtering methods (KNN-Item and SVD) provide context for the comparison, while MF and NeuMF are evaluated with both BCE and Focal Loss to test hypotheses H1 and H3.
...
\begin{tabular}{llll}
\hline
\textbf{Model} & \textbf{Type} & \textbf{Architecture} & \textbf{Loss Functions} \\
\hline
KNN-Item & Traditional & Item-based neighborhood & Cosine similarity \\
SVD & Traditional & Matrix factorization & MSE \\
MF & Neural & Dot-product embeddings & BCE, FL \\
NeuMF & Neural & GMF + MLP hybrid & BCE, FL \\
\hline
\end{tabular}
\end{table}
...
Latent factors & SVD & \{32, 64, 128\} & Embedding dimension \\
...
$\gamma$ (focusing) & \{0, 0.5, 1.0, 2.0, 3.0\} & $\gamma=0$ reduces to BCE \\
$\alpha$ (balancing) & \{0.25, 0.5, 0.75\} & Class weight balance \\
\hline
\end{tabular}
\end{table}
...
We report HR@10 and NDCG@10 as primary metrics.
...
\textbf{Convergence Metrics:} To assess whether Focal Loss provides computational efficiency benefits through improved gradient signal quality, we additionally report epochs to convergence (defined as the epoch achieving best validation NDCG@10) and total training time. We also visualize training loss and validation NDCG@10 curves over epochs for BCE vs. Focal Loss to illustrate the focusing effect on optimization trajectory.
...
\begin{itemize}
    \item \textbf{Multiple Seeds:} Each configuration is run with 5 different random seeds.
...
To isolate the contribution of Focal Loss hyperparameters and test hypothesis H2, we conduct the following ablation studies:

\begin{enumerate}
    \item \textbf{Focusing Parameter ($\gamma$) Ablation:} Vary $\gamma \in \{0, 0.5, 1.0, 2.0, 3.0\}$ with fixed $\alpha=0.25$ to analyze the focusing effect. Note that $\gamma=0$ reduces Focal Loss to standard $\alpha$-balanced BCE, serving as a direct comparison point.

    \item \textbf{Class-Balancing Parameter ($\alpha$) Ablation:} Vary $\alpha \in \{0.25, 0.5, 0.75\}$ with fixed $\gamma=2.0$ to analyze the class-balancing effect on positive/negative sample weighting.
\end{enumerate}

These ablations are conducted on NeuMF using the ML-1M dataset, which provides sufficient scale for meaningful hyperparameter sensitivity analysis.
