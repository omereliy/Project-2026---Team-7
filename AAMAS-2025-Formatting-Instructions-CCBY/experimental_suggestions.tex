...
\textbf{WARNING: Notebook execution incomplete.} The experimental notebook (\texttt{ml100k\_improved.ipynb}) stopped at cell 19 due to a scipy API deprecation (\texttt{binom\_test} removed). The Wilcoxon signed-rank test was NOT executed. All empirical results below are \textbf{single-seed only} and require statistical validation.

\textbf{Key findings from completed experiments:}
\begin{itemize}
    \item Default FL ($\gamma=2.0$, $\alpha=0.25$) \textbf{only improves at 1:50} (+26.79\% NDCG)
    \item At 1:4 and 1:10, default FL \textbf{underperforms} BCE (-5.62\% and -2.59\%)
    \item $\alpha$-BCE outperforms FL at 1:10, suggesting class weighting is key
    \item With tuned hyperparameters, FL can beat BCE at all ratios
\end{itemize}

\textbf{Required next steps:}
\begin{enumerate}
    \item Fix scipy: Replace \texttt{stats.binom\_test()} with \texttt{stats.binomtest()}
    \item Complete 10-seed Wilcoxon test at 1:50 sampling
    \item Test H3 (mechanism) at 1:50 where FL shows promise
\end{enumerate}
...
Robustness study (1:4, 1:10, 1:50) & Complete & H2 NOT SUPPORTED (1/3 ratios) \\
Grid search (36 configs) & Complete & 47\% win rate \\
Statistical analysis (binomial) & \textbf{FAILED} & scipy API deprecated \\
Wilcoxon test (10-seed) & \textbf{NOT RUN} & Blocked by above \\
\bottomrule
\end{tabular}
\end{center}

\subsection*{Hypothesis Status (Single-Seed, NOT Validated)}

\begin{description}
    \item[H1 (Efficacy):] \textbf{PARTIAL} --- NOT SUPPORTED at 1:10 (-2.59\%), SUPPORTED at 1:50 (+26.79\%)
    \item[H2 (Robustness):] \textbf{NOT SUPPORTED} --- FL wins at only 1/3 sampling ratios with default params
    \item[H3 (Mechanism):] \textbf{NOT SUPPORTED at 1:10} --- $\alpha$-BCE beats FL by 5.69\%; needs testing at 1:50
\end{description}
...
\begin{enumerate}
    \item \textbf{Focus on 1:50 results} where FL shows clear improvement (+26.79\%)
    \item \textbf{Be transparent about hyperparameter sensitivity} --- default CV params don't transfer
    \item \textbf{Reframe contribution} as understanding \emph{when} FL helps (extreme imbalance), not claiming universal improvement
    \item \textbf{Complete statistical validation} before making publication claims
    \item \textbf{Test H3 at 1:50} to determine if focusing mechanism matters at extreme imbalance
\end{enumerate}

\subsection*{Required Actions}

\begin{enumerate}
    \item Fix scipy error: \texttt{stats.binom\_test()} $\rightarrow$ \texttt{stats.binomtest()}
    \item Re-run notebook from cell 19
    \item Complete 10-seed Wilcoxon test at 1:50
    \item Update results and paper based on validated findings
\end{enumerate}
