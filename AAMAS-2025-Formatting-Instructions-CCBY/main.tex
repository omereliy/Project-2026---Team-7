...
\begin{description}
    \item[H1 (Focal Loss improves NeuMF):] NeuMF trained with Focal Loss achieves statistically significantly higher Hit Rate@10 and NDCG@10 compared to NeuMF trained with Binary Cross-Entropy loss. \omer{Issue: No effect size threshold specified. With 10 seeds, tiny improvements can be statistically significant but practically meaningless. Recommendation: Define minimum meaningful improvement (e.g., $\geq$1\% relative improvement in HR@10) or report effect sizes (Cohen's d) alongside p-values.}
...
\begin{enumerate}
    ...
    \item \textbf{Negative Sampling:} During training, sample 4 negative items per positive interaction. During evaluation, rank each test item against 99 randomly sampled negative items. \omer{Issue: 4:1 sampling ratio significantly reduces natural imbalance (14.9:1 to 21.4:1 per Table 2), potentially undermining Focal Loss's core advantage. Recommendation: Consider making higher sampling ratios (1:10 or 1:50) the primary experiment rather than ablation, since Focal Loss is designed for extreme imbalance scenarios.}
\end{enumerate}
...
$\alpha$ (balancing) & \{0.25, 0.5, 0.75\} & Class weight \omer{Issue: $\alpha$ values combined with 4:1 negative sampling may cancel out the imbalance effect. With $\alpha=0.5$ and 4:1 sampling, effective weighting becomes nearly balanced, undermining Focal Loss motivation. Recommendation: Analyze $\alpha \times$ sampling\_ratio interaction explicitly, or test with higher sampling ratios (1:10, 1:50) as primary experiments.} \\
\hline
\end{tabularx}
\end{table}
...
\textbf{Evaluation Procedure:} Following the leave-one-out protocol, for each user, we rank the held-out positive item against 99 randomly sampled negatives (100 items total) and compute metrics based on the positive item's rank. \omer{Issue: Random negative sampling in evaluation produces mostly easy negatives, which may not fully reveal Focal Loss's advantage in distinguishing hard cases. Recommendation: Consider adding full-ranking evaluation (rank against all items) or a hard-negative evaluation protocol as supplementary analysis.}
...
\omer{Missing Training Dynamics Analysis: The core claim is that Focal Loss shifts gradient mass from easy to hard examples. Recommendation: Add analysis tracking (1) per-sample loss contributions binned by prediction confidence, (2) gradient magnitude from easy vs. hard examples over epochs, (3) learning curves showing how quickly the model stops learning from easy negatives. This directly validates the mechanism, not just the outcome.}

To isolate the contribution of each component, we conduct the following ablation studies:

\begin{enumerate}
    \item \textbf{Loss Function Ablation:} Compare BCE, BPR, and Focal Loss on fixed NeuMF architecture to isolate the effect of the loss function. \omer{Issue: Unfair hyperparameter comparison - BCE has 0 extra params, Focal Loss has 2 ($\gamma$, $\alpha$). Recommendation: Add $\alpha$-balanced BCE ($\gamma=0$, $\alpha$ tuned) as control to isolate focusing effect from class weighting effect.}
