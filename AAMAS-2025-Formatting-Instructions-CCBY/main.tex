...
\title[Focal Loss for NCF]{A Comparative Analysis of Neural Collaborative Filtering and Standard Recommendation Systems using Focal Loss} % Change the title
...
\begin{itemize}
    ...
    \item We conduct an architecture-agnostic analysis by testing Focal Loss on both linear (MF) and non-linear (NeuMF) models, addressing whether benefits are specific to neural architectures.
...
\begin{enumerate}
    ...
    \item \textbf{Architecture-Agnostic Analysis:} By testing Focal Loss on both MF and NeuMF, we determine whether performance gains are specific to neural architectures or generalize across model complexity levels.
...
\begin{description}
    ...
    \item[H2 (Focal Loss improves MF):] Matrix Factorization trained with Focal Loss achieves statistically significantly higher ranking metrics compared to MF trained with BPR loss.
    \item[H3 (Neural methods outperform traditional):] NeuMF with Focal Loss outperforms traditional collaborative filtering methods (KNN, SVD) on ranking metrics.
    \item[H4 (Architecture gap reduction):] The performance gap between NeuMF and MF decreases when both are trained with Focal Loss compared to their respective default losses.
    \item[H5 (Optimal focusing parameter):] There exists an optimal value of the focusing parameter $\gamma$ in the range $[0.5, 3.0]$ that maximizes recommendation performance.
    \item[H6 (Imbalance sensitivity):] Focal Loss provides larger relative improvements on datasets with higher class imbalance ratios.
    \item[H7 (Sampling robustness):] Focal Loss maintains stable performance across varying negative sampling ratios (1:4, 1:10, 1:50), while BCE-trained models exhibit performance degradation as the ratio increases due to being overwhelmed by easy negatives.
\end{description}
...
\label{sec:baselines}
\omer{need to align the table, it overlaps with the 4.7 section header}
...
KNN-User & Traditional & User-based neighborhood & Cosine similarity \\
...
We report results for $K \in \{5, 10, 20\}$, with HR@10 and NDCG@10 as primary metrics.
