...
\omer{omer comment example}
\guy{guy comment example}
\rotem{rotem comment example}
\dvir{dvir comment example}
\end{abstract}
...
\todo{Write your introduction here} 
...
Both BCE and BPR treat all samples equally regardless of prediction confidence. In imbalanced settings, this causes easy negatives (items clearly irrelevant to a user) to dominate gradient updates while contributing minimal learning signal. Focal Loss~\cite{lin2017focal} addresses this by introducing a modulating factor that down-weights well-classified examples:
\begin{equation}
...
\begin{enumerate}
    \item \textbf{Binarization:} Convert explicit ratings to implicit feedback by treating ratings $\geq 4$ as positive interactions ($y=1$) and all others as negative ($y=0$).
...
We compare the following models, summarized in Table~\ref{tab:models}:
...
Table~\ref{tab:ablation-matrix} summarizes the ablation configurations.
